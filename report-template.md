...
Machine Learning Model 2 appears to perform better for the task of predicting loan risk (healthy vs. high-risk loans). The reasons for its superior performance are highlighted through several key indicators:
...
4. The F1-Score for high-risk loans in Model 2 improved to 0.91 from 0.88 in Model 1. The F1-Score is a harmonic mean of precision and recall, offering a single metric to assess a model's balanced performance between these two aspects. An improvement in the F1-Score for high-risk loans in Model 2 indicates a better balance between precision and recall, emphasizing its efficiency in identifying high-risk loans accurately.
